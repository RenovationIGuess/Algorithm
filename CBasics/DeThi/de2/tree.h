...
treetype Search(char *x, treetype t)
{
    ...
    else if(strcmp(t->element.username, x) == 0) return t;
    else if(strcmp(t->element.username, x) < 0) return Search(x, t->right);
...
void InsertNode(ElmType x, treetype *t)
{
    ...
    else if(strcmp(x.username, (*t)->element.username) < 0) InsertNode(x, &(*t)->left);
    else if(strcmp(x.username, (*t)->element.username) > 0) InsertNode(x, &(*t)->right);
    //else return t;
}
...
void DeleteNode(char *x, treetype *t)
{
    if((*t) != NULL)
    {
        if(strcmp(x, (*t)->element.username) < 0) DeleteNode(x, &(*t)->left);
        else if(strcmp(x, (*t)->element.username) > 0) DeleteNode(x, &(*t)->right);
        else if(IsLeaf((*t)))
        {
            (*t) = NULL;
        }
