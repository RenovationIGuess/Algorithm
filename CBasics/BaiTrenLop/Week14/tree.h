...
/*treetype Search(char *x, treetype t)
{
    ...
    else if(strcmp(t->element.email, x) == 0) return t;
    else if(strcmp(t->element.email, x) < 0) return Search(x, t->right);
...
void InsertNode(ElmType x, treetype *t)
{
    ...
    else if(strcmp(x.email, (*t)->element.email) < 0) InsertNode(x, &(*t)->left);
    else if(strcmp(x.email, (*t)->element.email) > 0) InsertNode(x, &(*t)->right);
    //else return t;
} */
...
void Breadth_first(treetype node)
{
    ...
    if(node != NULL)
    {
        ...
        while(!IsEmpty(&q))
        {
            ...
            if(node->left != NULL) EnQueue(node->left, &q);
            if(node->right != NULL) EnQueue(node->right, &q);
        }
    }
}
...
void DeleteNode(ElmType x, treetype *t)
{
    if((*t) != NULL)
    {
        if(x < (*t)->element) DeleteNode(x, &(*t)->left);
        else if(x > (*t)->element) DeleteNode(x, &(*t)->right);
        else if(IsLeaf((*t)))
        {
            (*t) = NULL;
        }
