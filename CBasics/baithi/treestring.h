...
int Compare(ElmType a, ElmType b)
{
    //ket qua cua phep so sanh
    int res;
    //luu ten a cua moi lan check
    char *s1 = (char *)malloc(MAX * sizeof(char));
    //luu ten b cua moi lan check
    char *s2 = (char *)malloc(MAX * sizeof(char));
    //so ky tu con lai de xet cua a
    int len1 = strlen(a.name);
    strcpy(s1, getname(a.name, len1));
    //so ky tu con lai de xet cua b
    int len2 = strlen(b.name);
    strcpy(s2, getname(b.name, len2));
    len1 -= (strlen(s1) + 1);
    len2 -= (strlen(s2) + 1);
    while((res = strcmp(s1, s2)) == 0 && len1 > 0 && len2 > 0)
    {
...
int LowerThankey(ElmType x, treetype t)
{
    treetype p = t;
    int cnt = 0;
    while(t)
    {
        if(t->left == NULL)
        {
            if(t->element.point <= x.point && Compare(x, t->element) >= 0)
            {
...
int HigherThankey(ElmType x, treetype t)
{
    treetype p = t;
    int cnt = 0;
    while(t)
