s1 = ""
s2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
s3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
s4 = ""
s5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
s6 = ""
s7 = ""
s8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
s9 = ""
s0 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
s10 = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"

def decode(s):
	d = []
	for i in range(len(s)//2):
		d.append(int(s[i*2:i*2+2],16))
	return(d)
def chuyen_sang_he_10():
	X = []
	for i in range(11):
		k = eval("decode(s" + str(i) + ")")
		X.append(k)
	return(X)
def vi_tri_space_2s(x1,x2):
	d = [0]*len(x1)
	if len(x1) > len(x2):
		for i in range(len(x2)):
			if 97 <= x1[i]^x2[i] <= 122 or 65 <= x1[i]^x2[i] <= 90 or x1[i]^x2[i] == 0:
				d[i] = 1
		for i in range(len(x2),len(x1)):
			d[i] = 1
		#Nếu x1 dài hơn thì các vị trí không xác định mặc định để nó là dấu cách tiện cho việc tìm vị trí.
	else:
		for i in range(len(x1)):
			if 97 <= x1[i]^x2[i] <= 122 or 65 <= x1[i]^x2[i] <= 90 or x1[i]^x2[i] == 0:
				d[i] = 1
	return(d)
def vi_tri_space_1s(d_space):
	len_d_space0 = len(d_space[0])
	D = [1]*len_d_space0
	for i in range(len(d_space)):
		for j in range(len_d_space0):
			D[j] += d_space[i][j]
	for i in range(len_d_space0):
		if D[i] > 9:
			D[i] = 1
		else:
			D[i] = 0
	return(D)
def tim_key_cua_1s(x, d_space):
	key_1s = []
	for i in range(len(d_space)):
		if d_space[i] == 1:
			key_1s.append(x[i]^32)
		else:
			key_1s.append("")
	return(key_1s)
def tim_key(X, d_space_all):
	key = [0]*300
	d_key = []
	for i in range(11):
		d_key.append(tim_key_cua_1s(X[i], vi_tri_space_1s(d_space_all[i])))
	for d in d_key:
		for i in range(len(d)):
			if d[i] != "":
				key[i] = d[i]
	return(key)
def giai_ma(x,key):
	sx = ''
	for i in range(len(x)):
		if key[i] != 0:
			sx += str(chr(x[i]^key[i]))
		else:
			sx += "*"
	return(sx)
def main():
	X = chuyen_sang_he_10()
	#d_space_all là danh sách các vị trí của cặp s1 với s2,3,4,5,6... , s2 với s2,3,4,5,6..., ....
	d_space_all = []
	for i in range(11):
		d_space = []
		for j in range(11):
			if i != j:
				d_space.append(vi_tri_space_2s(X[i],X[j]))
		d_space_all.append(d_space)

	key = tim_key(X, d_space_all)
	for i in range(11):
		print(giai_ma(X[i],key))

main()
